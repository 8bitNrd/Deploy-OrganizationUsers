...
$topLevelDomain = ($env:USERDNSDOMAIN).Split(".")[1]

#Functions
Function Renadom-FirstName 
{
...
function New-Employee
{
   ...
   $EmployeeData = New-Object psobject -Property @{
      ...
      GivenName = Renadom-FirstName
      Surname = Renadom-LastName
   }
   ...
   return New-ADUser -Name ($EmployeeData.GivenName+" "+$EmployeeData.surName)-GivenName $EmployeeData.GivenName -Surname $EmployeeData.surName -AccountPassword (ConvertTo-SecureString $EmployeeData.password -AsPlainText -Force) -OfficePhone $phone -Enabled $true -Department $Department -SamAccountName $SamAccountName -EmployeeNumber $EmployeeNumber -DisplayName ($EmployeeData.GivenName+" "+$EmployeeData.surName) -EmailAddress ($EmployeeData.GivenName+"."+$EmployeeData.surName+"@"+$domainName+".$topLevelDomain") -Path $Path -PassThru
}
...
Foreach ($deparment in $departmentList)
{
   ...
   #Creating Managers
   [Int]$managerCount = 0
   While ([Int]$managerCount -lt $ManagersPerDepartment)
   {
